...
'상향식' 접근 방식은 '하향식' 접근 방식에 비해 공식적으로 덜 정의되어 있음.
데이터베이스 설계 및 구현의 세부적인 측면에 중점을 두는 사고방식과 일련의 관행을 포괄합니다. 이러한 관행은 데이터 모델링의 보다 개념적인 단계에 선행하거나 동시에 실행되는 경우가 많습니다. '상향식'으로 특별히 분류되는 보편적으로 인정되는 3단계 방법은 없지만, 세부적인 설계, 최적화 및 구현에 중점을 둔다는 점에서 이 접근 방식을 이해할 수 있습니다. 다음은 데이터베이스 설계의 다양한 측면과 관련된 방식입니다:

세부 설계 및 최적화
엔티티-속성-값(EAV) 모델링: EAV는 많은 수의 속성으로 설명되지만 일부 속성만 값을 갖는 엔티티를 공간 효율적인 방식으로 인코딩하는 데이터 모델입니다. 이 모델은 희소 데이터를 다루거나 각 엔티티의 정확한 속성 집합을 알 수 없거나 매우 가변적일 때 상향식 접근 방식에 특히 유용합니다.
인덱싱 전략: 최적의 인덱스 집합을 결정하는 것은 상향식 접근 방식에서 매우 중요한 부분입니다. 인덱스는 쿼리 성능에 상당한 영향을 미칠 수 있으므로 이 단계에서는 일반적인 쿼리를 분석하고 이를 지원하기 위해 데이터를 인덱싱하는 가장 효율적인 방법을 파악해야 합니다.
파티셔닝 및 샤딩: 대규모 데이터베이스는 종종 더 작고 관리하기 쉬운 조각으로 나누면 이점이 있습니다. 파티셔닝은 테이블을 더 작고 관리하기 쉬운 조각으로 나누고, 샤딩은 여러 서버에 데이터를 분산시킵니다. 두 가지 전략 모두 성능과 확장성에 중점을 둔 상향식 접근 방식의 일부입니다.
구현 및 테스트
스키마 생성 및 마이그레이션: 상향식 접근 방식에서는 데이터베이스 스키마를 생성하기 위해 실제 SQL 스크립트 또는 데이터베이스 관리 시스템(DBMS) 명령이 생성됩니다. 여기에는 테이블 생성, 관계 정의, 제약 조건 설정 등이 포함됩니다. 또한 기존 데이터를 새로운 스키마 형식으로 변환하기 위해 마이그레이션 스크립트가 개발됩니다.
성능 조정: 초기 구현 후에는 최적의 성능을 위해 데이터베이스를 튜닝하는 상향식 접근 방식이 강조됩니다. 여기에는 쿼리 성능 분석, 인덱스 조정, 구성 최적화가 포함됩니다.
백업 및 복구 계획: 데이터 안전을 위해서는 강력한 백업 및 복구 전략을 수립하는 것이 중요합니다. 상향식 접근 방식에는 백업 빈도, 백업 유형(전체, 차등, 증분), 복구 절차 등 이러한 전략을 세부적으로 수립하는 것이 포함됩니다.
다른 접근 방식과의 통합
상향식 접근 방식이 하향식 접근 방식의 중요성을 부정하는 것이 아니라 오히려 이를 보완한다는 점에 유의하는 것이 중요합니다. 전체적인 데이터베이스 설계 프로세스에는 개념 모델링(하향식)과 세부 설계 및 구현(상향식) 간의 반복이 수반되는 경우가 많습니다. 개념 모델은 프레임워크와 높은 수준의 구조를 제공하는 반면, 상향식 접근 방식은 세부 사항을 채우고, 성능을 최적화하며, 구현을 준비합니다.

요약하면, 데이터 모델링에서 상향식 접근 방식에 대한 공식적인 '3단계' 방법은 없지만, 이 접근 방식의 본질은 세부적이고 구현에 중점을 둔다는 데 있습니다. 이 접근법은 최적화, 실용성, 데이터베이스 설계 및 구현의 기술적 측면을 강조합니다.
...
- 성능: 특히 대규모 데이터 세트나 복잡한 쿼리를 처리할 때 RDB에서 성능 문제가 발생할 수 있습니다. 성능을 개선하려면 쿼리를 인덱싱하고 최적화해야 할 수도 있습니다.
