...
> 2024/07/10: 하향식 설계, 디자인 절차 추가, 보완  
...
실제로 해보니 좀 더 구체적이고, 체계적인 절차 설명이 필요하다고 느낌.
...
2. 개념적 설계 Conceptual Design: 엔티티-관계 다이어그램(ERD)을 만듦.
	...
	2-4. 정규화: 데이터베이스를 정규화하여 중복성을 제거하고 데이터 무결성을 보장 필요. 정규화 형식(1NF, 2NF, 3NF, BCNF 등)에 따라 데이터 정리, 중복 제거. 각 테이블에 하나의 엔티티에 대한 데이터, 관계가 올바르게 관리되는지 확인
...
6. 테스트 및 유효성 검사 Testing and Validation: 샘플 데이터로 데이터베이스를 테스트하여 요구 사항을 충족하는지 확인. 데이터와 관계의 무결성을 검증

7. 유지 관리 및 최적화 Maintenance and Optimization: 데이터베이스 성능을 모니터링하고 필요한 최적화를 수행합니다.
데이터베이스를 정기적으로 백업합니다.
애플리케이션 요구 사항이 변화함에 따라 데이터베이스 스키마를 업데이트합니다.

- 성능 고려: 설계 단계 초기에 쿼리 성능을 고려. 전략적으로 열 Column을 색인화(인덱싱)하면 데이터 검색 속도를 크게 높일 수 있음.
