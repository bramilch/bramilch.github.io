...
- 정규화는 일반적으로 일련의 단계를 따르며, 각 단계는 정규화의 형태에 해당. 

- 1NF First Normal Form: 동일한 테이블에서 중복 열을 제거하고, 관련 데이터의 각 그룹에 대해 별도의 테이블을 만들고, 각 행을 고유한 열 또는 열 집합(기본 키)으로 식별합니다.

- 2NF Second Normal Form: 모든 비키 속성이 기본 키에 완전히 종속되도록 하기 위해 테이블을 별도의 테이블로 더 세분화합니다.

- 3NF Third Normal Form: 기본 키에 종속되지 않는 열을 제거합니다.
보이스-코드 일반 형식(BCNF): 3NF의 일부 문제, 특히 복합 키와 관련된 문제를 해결합니다.

- BCNF Boyce-Codd Normal Form: 테이블이 3NF인지 확인
모든 결정자는 후보 키여야 합니다. 결정자는 다른 속성이 완전히 기능적으로 의존하는 속성입니다.

- 4NF Fourth Normal Form: 다중 값 종속성을 처리합니다.

- 5NF) Fifth Normal Form 프로젝트-조인 정규 형식(PJ/NF)이라고도 하며, 특정 유형의 투영 및 조인 작업에서 보존되지 않는 함수 종속성 문제를 해결합니다.
