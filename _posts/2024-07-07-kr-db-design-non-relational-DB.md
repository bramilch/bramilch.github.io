...
- 유연성 Flexibility: NoSQL 데이터베이스는 ***스키마가 없는 데이터 모델을 제공***하므로 보다 유연하고 동적인 데이터 구조가 가능

- 확장성 Scalability: 수평적으로 확장할 수 있도록 설계되어 분산된 시스템에서 대규모 데이터를 보다 쉽게 처리할 수 있음.

- 성능 Performance: ***높은 읽기/쓰기 처리량에 최적화***된 NoSQL 데이터베이스는 ***특정 워크로드***에 더 빠른 성능을 제공할 수 있음.
...
- 컬럼-패밀리 방식(Cassandra, HBase): **쓰기 작업이 많은 워크로드, 대규모 데이터 세트 및 넓은 컬럼 스토리지**에 적합
...
비관계형 데이터베이스 사용 사례
실시간 분석: 시계열 데이터와 실시간 분석을 처리하기 위해 Cassandra와 같은 컬럼 패밀리 저장소를 사용하기.
콘텐츠 관리: 유연하고 확장 가능한 콘텐츠 관리 시스템을 위해 MongoDB와 같은 문서 저장소를 사용합니다.
캐싱: 자주 액세스하는 데이터를 캐싱하여 성능을 향상시키기 위해 Redis와 같은 키-값 저장소를 활용합니다.
소셜 네트워크: Neo4j와 같은 그래프 데이터베이스를 활용하여 소셜 네트워크의 복잡한 관계를 관리하고 쿼리합니다.
관계형 데이터베이스 사용 사례
...
실제 업무에서 두 가지를 모두 사용하는 방법

## Polyglot Persistence 다양한 언어, 도구 지속성


> polyglot [adjective](https://www.merriam-webster.com/dictionary/adjective)
...
Polyglot Persistence 다언어 지속성이란 동일한 애플리케이션 내에서 서로 다른 작업에 최적화된 여러 데이터베이스를 사용하는 관행입니다. 이를 통해 관계형 데이터베이스와 비관계형 데이터베이스의 강점을 모두 활용할 수 있습니다.

통합을 위한 실용적인 단계
사용 사례 파악: 애플리케이션의 어떤 부분(예: 대용량 로깅)이 NoSQL의 이점을 누릴 수 있는지, 어떤 부분(예: 금융 거래)에 관계형 데이터베이스의 ACID 속성이 필요한지 결정합니다.

데이터 흐름 설계:

데이터 세분화: 사용량에 따라 데이터를 세분화합니다. 예를 들어, 사용자 프로필은 문서 저장소에 저장하고 트랜잭션 데이터는 관계형 데이터베이스에 저장할 수 있습니다.
ETL 프로세스: ETL(추출, 변환, 로드) 프로세스를 사용해 필요에 따라 시스템 간에 데이터를 이동합니다. Apache Kafka와 같은 도구는 실시간 데이터 통합을 촉진할 수 있습니다.
데이터 일관성: 중요하지 않은 데이터에는 이벤트 일관성을, 중요한 작업에는 강력한 일관성을 사용하는 등 시스템 간의 데이터 일관성을 위한 전략을 구현하세요.
API 계층:

통합 API: 기본 데이터베이스 시스템을 추상화하는 통합 API 계층을 만듭니다. 이를 통해 애플리케이션이 서로 다른 데이터베이스와 원활하게 상호 작용할 수 있습니다.
마이크로서비스: 마이크로 서비스: 각 서비스가 요구 사항에 가장 적합한 데이터베이스를 사용할 수 있는 마이크로 서비스 아키텍처를 활용하세요.
동기화 메커니즘:

변경 데이터 캡처(CDC): CDC를 구현하여 데이터의 변화를 모니터링하고 이에 대응하세요. Debezium과 같은 도구는 관계형 데이터베이스의 변경 사항을 캡처하여 NoSQL 시스템으로 전파할 수 있습니다.
2단계 커밋(2PC): 데이터베이스 전반에서 강력한 일관성이 필요한 중요한 작업의 경우, 2PC를 사용하여 모든 시스템에서 트랜잭션이 커밋되도록 하거나 전혀 커밋되지 않도록 할 수 있습니다.
데이터 액세스 패턴:

하이브리드 쿼리: 데이터베이스가 가장 잘하는 작업에 데이터베이스를 사용하세요. 예를 들어, 관계형 데이터베이스에서는 복잡한 조인을 수행하고 NoSQL 데이터베이스에서는 간단한 키-값 쌍을 검색하세요.
캐싱: 관계형 데이터베이스에서 자주 액세스하는 데이터를 Redis와 같은 키-값 저장소에 캐싱하는 캐싱 메커니즘을 구현하세요.
