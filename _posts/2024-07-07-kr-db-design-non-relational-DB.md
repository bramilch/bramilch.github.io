...
- 샤딩: 샤딩: 샤드 키를 기반으로 여러 서버에 데이터를 분산하는 것입니다. 이를 통해 수평적 확장 및 로드 밸런싱이 가능합니다.

- 복제: 복제: 여러 서버에 데이터 사본을 생성하여 데이터 가용성과 내결함성을 보장합니다.

- 파티셔닝: 데이터를 더 작고 관리하기 쉬운 부분으로 분할하여 쿼리 성능을 최적화하고 경합을 줄입니다.
