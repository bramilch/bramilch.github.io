...
|카테고리| 패턴 |설명|
|------------------|-------------------|------------------------|
|생성| 싱글톤 |클래스에 인스턴스가 하나만 있도록 하고 전역 액세스 지점을 제공합니다.|
|생성| 팩토리 메서드| 객체를 생성하기 위한 인터페이스를 정의하지만 서브클래스가 인스턴스화할 클래스를 결정할 수 있도록 합니다.|
|생성| 추상 팩토리| 구체적인 클래스를 지정하지 않고 관련 또는 종속 객체의 패밀리를 생성하기 위한 인터페이스를 제공합니다.|
|생성|빌더 |복잡한 객체의 구성과 그 표현을 분리합니다.|
|생성|프로토타입 |프로토타입 인스턴스를 사용하여 생성할 객체의 종류를 지정하고 이 프로토타입을 복사하여 새 객체를 생성합니다.|
|구조| 어댑터| 한 클래스의 인터페이스를 클라이언트가 기대하는 다른 인터페이스로 변환합니다.|
|구조|브리지 |추상화와 구현을 분리하여 두 가지가 독립적으로 변할 수 있도록 합니다.|
|구조| 합성| 객체를 트리 구조로 구성하여 부분 전체 계층 구조를 나타냅니다.|
|구조| 데코레이터 |오브젝트에 동적으로 추가 책임을 부여합니다.|
|구조| 파사드| 하위 시스템의 인터페이스 세트에 통합된 인터페이스를 제공합니다.|
|동작| 책임| 체인 오브젝트가 요청을 처리할 때까지 잠재적 처리기 체인을 따라 요청을 전달합니다.|
|동작| 명령| 요청을 객체로 캡슐화하여 사용자가 대기열, 요청 및 작업으로 클라이언트를 매개변수화할 수 있도록 합니다.|
|동작| 인터프리터| 복합 구조로 표현되는 문법에 대한 인터프리터를 제공합니다.|
|동작| 이터레이터 |집합 객체의 요소에 순차적으로 액세스하는 방법을 제공합니다.|
|동작| 매개자| 객체 집합이 상호 작용하는 방식을 캡슐화하는 객체를 정의합니다.|
|동작| 메멘토| 나중에 개체를 이 상태로 복원할 수 있도록 개체의 상태를 캡처하고 내재화합니다.|
|동작| 관찰자| 객체 간의 일대다 종속성을 정의하여 한 객체의 상태가 변경되면 모든 종속 객체에 알림이 전송되고 자동으로 업데이트되도록 합니다.|
|동작| 상태| 내부 상태가 변경되면 개체의 동작이 변경되도록 합니다.|
|동작| 전략| 알고리즘 제품군을 정의하고 각 알고리즘을 캡슐화하여 상호 교환할 수 있도록 합니다.|
|동작| 템플릿 메서드 |연산에서 알고리즘의 골격을 정의하여 일부 단계를 하위 클래스로 연기합니다.|
|동작| 방문자| 객체 구조의 요소에 대해 수행될 연산을 나타냅니다.|
