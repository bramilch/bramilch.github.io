...
- 모놀리식 커널은 핵심 기능과 모든 드라이버를 포함한 전체 운영 체제가 커널 모드에서 실행되는 커널 유형입니다. 이 설계는 모든 시스템 서비스를 단일 주소 공간에서 실행되는 하나의 큰 코드 블록으로 통합합니다.


**특징**

단일 주소 공간: 모든 운영 체제 서비스(예: 장치 드라이버, 파일 시스템 관리, 메모리 관리 등)가 동일한 주소 공간에서 실행됩니다.

- 성능: 대부분의 시스템 호출에서 사용자 모드와 커널 모드 간에 컨텍스트 전환이 필요하지 않으므로 모놀리식 커널은 매우 빠를 수 있습니다.

- 복잡성: 커널의 크기가 크고 서비스가 통합되어 있어 커널이 복잡하고 관리하기 어려울 수 있습니다.

**장점**

- 고성능: 커널 공간 내에서 직접 통신하므로 시스템 호출을 빠르게 실행할 수 있습니다.
설계의 단순성: 모든 것이 하나의 큰 프로그램의 일부이므로 초기에 설계하고 구현하기가 더 쉽습니다.

**단점**

- 안정성 및 보안 위험: 커널의 어느 부분에서든 버그가 발생하면 전체 시스템이 다운되거나 보안이 손상될 수 있습니다.

- 유지 관리: 크기와 상호 의존성으로 인해 유지 관리 및 업데이트가 어렵습니다.

**예시**

Linux 커널: Linux 운영 체제는 모놀리식 커널을 사용합니다.

<br>

### 2. 마이크로커널(새뮤엘 커널) Microkernels (or Samuel Kernel)

<br>


- 마이크로커널(또는 사무엘 커널)은 커널 모드에서 실행되는 기능을 최소화하는 것을 목표로 하는 커널의 한 유형입니다. 대신 장치 드라이버, 파일 시스템 및 네트워크 프로토콜과 같은 대부분의 서비스를 사용자 공간에서 실행하고 메시지 전달을 통해 통신합니다.


**특징**

- 최소한의 코어: 프로세스 간 통신, 기본 스케줄링, 메모리 관리와 같은 필수 서비스만 커널 모드에서 실행됩니다.

- 모듈화: 필수적이지 않은 서비스는 사용자 공간에서 별도의 프로세스로 실행됩니다.
프로세스 간 통신(IPC): 커널과 사용자 공간 서비스 간의 통신을 위해 메시지 전달에 크게 의존합니다.

**장점**

- 안정성 및 보안: 사용자 공간 서비스의 버그가 전체 시스템을 다운시키지 않으며 독립적으로 다시 시작할 수 있습니다.

- 모듈성 및 유연성: 전체 시스템에 영향을 주지 않고 개별 서비스를 쉽게 유지 관리, 업데이트 및 확장할 수 있습니다.

**단점**

- 성능 오버헤드: 잦은 컨텍스트 전환과 IPC로 인해 모놀리식 커널에 비해 성능 오버헤드가 발생할 수 있습니다.

- 구현의 복잡성: 효율적인 IPC 메커니즘이 필요하기 때문에 설계 및 구현이 더 복잡해집니다.

**예시**

- Minix: 마이크로커널 기반 운영 체제의 예입니다.
...
- 하이브리드 커널은 모놀리식 아키텍처와 마이크로 커널 아키텍처의 조합입니다. 일부 서비스는 성능상의 이유로 커널 모드(모놀리식 커널처럼)에서 실행하고, 다른 서비스는 모듈성과 안정성을 위해 사용자 공간(마이크로커널처럼)에서 실행하여 두 설계의 장점을 모두 활용하려고 합니다.

**특성**

- 혼합 접근 방식: 모놀리식 및 마이크로커널 설계의 요소를 모두 통합합니다.

- 선택적 서비스 배치: 성능에 중요한 서비스는 커널 모드에서 실행하고, 다른 서비스는 사용자 공간에서 실행할 수 있습니다.

- 유연성: 커널 모드에서 실행되는 서비스와 사용자 공간에서 실행되는 서비스를 조정하여 다양한 요구 사항에 맞게 조정할 수 있습니다.

**장점**

균형 잡힌 성능과 안정성: 모놀리식 커널의 고성능과 마이크로커널의 모듈성 및 안정성 사이에서 적절한 균형을 이룰 수 있습니다.

- 유연성: 커널과 사용자 공간 간에 서비스를 이동하여 특정 사용 사례에 맞게 더 쉽게 최적화할 수 있습니다.

**단점**

- 복잡성: 모놀리식 설계와 마이크로커널 설계 간의 균형을 맞추기 위해 설계 및 관리가 복잡할 수 있습니다.

- 문제 발생 가능성: 신중하게 설계하지 않으면 두 아키텍처의 단점을 모두 계승할 수 있습니다.

**예시**

- Windows NT: 최신 Windows 버전을 포함한 Windows NT 운영 체제는 하이브리드 커널을 사용합니다.
