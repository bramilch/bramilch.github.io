...
- pyenv의 경우, 공식 문서에서 windows를 공식적으로 지원하지 않고, Linux의 Windows 하위 시스템에서도 작동하지 않아 @kirankotari의 pyenv-win 포크해서 사용하라고 함([pyenv 공식문서 참고](https://github.com/pyenv/pyenv?tab=readme-ov-file#windows))
...
- ***Python 가상환경은 기본적으로 1. 특정 인터프리터, 2. 라이브러리 및 종속성, 3. 스크립트 또는 바이너를 포함한 독립된 디렉터리***
...
| **다국어 지원 (Cross-Language)**  | 지원하지 않음                   | 지원하지 않음                   | 지원하지 않음              | 지원하지 않음                        | 지원함                                | 지원하지 않음                          |
| **환경 활성화**                   | `source env/bin/activate`      | `source env/bin/activate`      | `workon <env_name>`        | `pyenv activate <env_name>`         | `conda activate <env_name>`           | `poetry shell` 또는 `poetry run`       |
| **환경 생성**                     | `python -m venv <env_name>`    | `virtualenv <env_name>`        | `mkvirtualenv <env_name>`  | `pyenv virtualenv <version> <env_name>` | `conda create -n <env_name>`          | `poetry init` 및 `poetry install`      |
| **격리 수준**                     | 파이썬 패키지만                 | 파이썬 패키지만                 | 파이썬 패키지만            | 파이썬 버전 및 패키지 관리           | 전체(언어, 버전, 패키지)               | 파이썬 패키지만                        |
...
| **프로젝트별 환경**                | 지원하지 않음                   | 지원하지 않음                   | 지원하지 않음              | `pyenv-virtualenv` 플러그인으로 지원 | 지원함                                | 지원함                                 |
